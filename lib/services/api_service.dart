    ...
    if (response.statusCode >= 400) {
      try {
        ...
        if (raw.contains('<!DOCTYPE html>') || raw.contains('<html')) {
          throw Exception('API route not found (${response.statusCode}). Please verify backend URL and endpoint.');
        }

        throw Exception('Request failed (${response.statusCode}): ${raw.toString().trim()}');
      }
    }
  }
}
