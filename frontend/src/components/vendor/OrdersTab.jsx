...
function OrdersTab({ orders, onConfirm, onGoToInvoices, focusLoiId, onClearFocus, onViewInvoice, onEditInvoice }) {
  ...
  const getNextStep = (status) => {
    if (status === 'pending') return 'Confirm the order to proceed.';
    if (status === 'confirmed') return 'Prepare delivery and await payment.';
    if (status === 'completed') return 'Order is complete.';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-500">Confirm and track orders from the purchase manager.</p>
      </div>
