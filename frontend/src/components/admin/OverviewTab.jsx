...
function OverviewTab({ overviewStats, onGoToRegistrations, onGoToVendorProducts }) {
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        <h2 className="text-2xl font-semibold text-slate-900">Overview</h2>
        <p className="text-sm text-slate-500">Snapshot of supplier activity and registrations.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <p className="text-xs font-semibold text-slate-500 uppercase">Total Suppliers</p>
          <p className="text-3xl font-bold text-slate-900 mt-2">{overviewStats.totalSuppliers}</p>
          <p className="text-xs text-slate-500 mt-2">All registered suppliers</p>
        </div>
        <div className="bg-white border border-emerald-200 rounded-2xl p-5 shadow-sm">
          <p className="text-xs font-semibold text-emerald-600 uppercase">Approved</p>
          <p className="text-3xl font-bold text-emerald-700 mt-2">{overviewStats.approvedSuppliers}</p>
          <p className="text-xs text-emerald-600 mt-2">
            {overviewStats.totalSuppliers > 0
              ? Math.round((overviewStats.approvedSuppliers / overviewStats.totalSuppliers) * 100)
              : 0}% of total
          </p>
        </div>
        <div className="bg-white border border-amber-200 rounded-2xl p-5 shadow-sm">
          <p className="text-xs font-semibold text-amber-600 uppercase">Pending</p>
          <p className="text-3xl font-bold text-amber-700 mt-2">{overviewStats.pendingSuppliers}</p>
        ...
        <div className="bg-white border border-rose-200 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-3xl font-bold text-rose-700 mt-2">{overviewStats.rejectedSuppliers}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
          <p className="text-xs font-semibold text-slate-500 uppercase">Vendor Products</p>
          <p className="text-2xl font-bold text-slate-900 mt-2">{overviewStats.totalVendorProducts}</p>
          <button
            onClick={onGoToVendorProducts}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Recent Supplier Registrations</h3>
            <p className="text-sm text-slate-500 mt-1">Latest applications from suppliers</p>
          </div>
          <button
            onClick={onGoToRegistrations}
