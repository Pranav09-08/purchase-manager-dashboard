          ...
          <div className="w-full max-w-4xl rounded-2xl bg-white shadow-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              {(() => {
                ...
                // Check for LOI to get advance payment percent
                const loi = lois.find(l => {
                  const loiOrderId = l.order_id || l.orderId;
                  return String(loiOrderId) === String(selectedInvoice.order_id);
                });
                const advancePercent = loi?.advance_payment_percent || 0;
                  ...
                  <div className="border-t border-slate-200 pt-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                      <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
                        <p className="text-xs text-blue-600">Advance Payment ({advancePercent}%)</p>
                        <p className="font-semibold text-blue-900">{formatCurrency(advanceAmount)}</p>
                        <p className="text-xs text-blue-700 mt-1">Paid: {formatCurrency(Math.min(advancePayments, advanceAmount))}</p>
                      </div>
                      <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                        <p className="text-xs text-emerald-600">Final Payment</p>
                        <p className="font-semibold text-emerald-900">{formatCurrency(finalAmount)}</p>
                      ...
                      <div className="bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-2">
                        ...
                        <p className={`font-semibold ${pending <= 0.01 ? 'text-emerald-700' : 'text-indigo-900'}`}>
                          {formatCurrency(pending)}
                        </p>
                        <p className="text-xs text-indigo-700 mt-1 font-semibold">{pending <= 0.01 ? 'Closed ✓' : 'Open'}</p>
                      </div>
                    </div>
                  </div>
                  </>
