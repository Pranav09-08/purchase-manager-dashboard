      ...
      {activeTab === 'create' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {formData.quotationId && quotations.find(q => q.quotation_id === formData.quotationId) && (
              <div className="md:col-span-2 bg-slate-50 rounded-lg p-4 border border-slate-200">
                <div className="mb-4">
                  <h4 className="text-sm font-semibold text-slate-700 mb-3">Quotation Details</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-slate-500">Status</p>
                      <p className="font-semibold text-slate-900">{quotations.find(q => q.quotation_id === formData.quotationId)?.status || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">Total Amount</p>
                      <p className="font-semibold text-slate-900">₹{Number(quotations.find(q => q.quotation_id === formData.quotationId)?.total_amount || 0).toLocaleString('en-IN')}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">Delivery Date</p>
                      <p className="font-semibold text-slate-900">{quotations.find(q => q.quotation_id === formData.quotationId)?.expected_delivery_date ? new Date(quotations.find(q => q.quotation_id === formData.quotationId).expected_delivery_date).toLocaleDateString() : 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-500">Advance %</p>
                      <p className="font-semibold text-slate-900">{quotations.find(q => q.quotation_id === formData.quotationId)?.advance_payment_percent || 0}%</p>
                    </div>
                  </div>
                </div>
                {quotations.find(q => q.quotation_id === formData.quotationId)?.items?.length > 0 && (
                  <div>
                    <h5 className="text-xs font-semibold text-slate-600 uppercase mb-2">Line Items</h5>
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-xs">
                        <thead className="text-xs uppercase bg-slate-100">
                          <tr className="border-b border-slate-200">
                            <th className="py-1 px-2 text-left">Component</th>
                        ...
                        <tbody>
                          {quotations.find(q => q.quotation_id === formData.quotationId)?.items?.map((item) => (
                            <tr key={item.item_id} className="border-b border-slate-100">
                              <td className="py-1 px-2 text-slate-700">{item.component_id}</td>
                              <td className="py-1 px-2 text-right text-slate-700">{item.quantity}</td>
                              <td className="py-1 px-2 text-right text-slate-700">₹{Number(item.unit_price || 0).toLocaleString('en-IN')}</td>
                              <td className="py-1 px-2 text-right font-semibold text-slate-900">₹{Number(item.line_total || 0).toLocaleString('en-IN')}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
