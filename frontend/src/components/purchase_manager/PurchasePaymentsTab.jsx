            ...
            <>
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {activeFormData.orderId && (() => {
                  ...
                  // Get LOI info for advance payment
                  const loi = invoices.length > 0 && invoices.find(inv => String(inv.order_id) === String(activeFormData.orderId));
                  ...
                  // Calculate paid advance and final
                      ...
                      <div className="md:col-span-2">
                        <div className="grid grid-cols-3 gap-3 text-sm mb-4">
                          <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
                            <p className="text-xs text-blue-600 font-semibold">Advance Payment ({advancePercent}%)</p>
                            <p className="font-bold text-blue-900">{formatCurrency(advanceAmount)}</p>
                            <p className="text-xs text-blue-700 mt-1">Paid: {formatCurrency(Math.min(advancePayments, advanceAmount))}</p>
                            <p className="text-xs text-blue-700">Remaining: {formatCurrency(Math.max(0, advanceAmount - advancePayments))}</p>
                          </div>
                          <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                            <p className="text-xs text-emerald-600 font-semibold">Final Payment</p>
                            <p className="font-bold text-emerald-900">{formatCurrency(finalAmount)}</p>
                            <p className="text-xs text-emerald-700 mt-1">Paid: {formatCurrency(Math.min(finalPayments, finalAmount))}</p>
                            <p className="text-xs text-emerald-700">Remaining: {formatCurrency(Math.max(0, finalAmount - finalPayments))}</p>
                          </div>
                          <div className="bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-2">
                            <p className="text-xs text-indigo-600 font-semibold">Total Summary</p>
                            <p className="font-bold text-indigo-900">{formatCurrency(totalAmount)}</p>
                            <p className="text-xs text-indigo-700 mt-1">Paid: {formatCurrency(paidAmount)}</p>
                            <p className="text-xs text-indigo-700 font-semibold">Remaining: {formatCurrency(remaining)}</p>
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    return (
                      <>
                        ...
                        <div>
                          ...
                          {activeFormData.orderId && activeFormData.amount && (
