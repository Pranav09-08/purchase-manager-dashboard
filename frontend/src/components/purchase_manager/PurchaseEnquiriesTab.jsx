...
function PurchaseEnquiriesTab({
  ...
  const filteredComponents = selectedVendorId
    ? components.filter((component) => {
      ...
      return String(componentVendorId) === String(selectedVendorId);
    })
    : [];
            ...
            {selectedItems.length === 0 ? (
              ...
              <div className="space-y-4">
                {selectedItems.map((item) => {
                  const component = components.find(c => (c.componentId || c.componentid || c.component_id || c.id) === item.componentId);
                  const componentDescription = component?.description || '';
                  const quantity = Number(item.quantity || 0);
                  const unitCost = Number(item.estimated_unit_cost || 0);
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                        <div>
                          ...
                          {showErrors && isQuantityInvalid(item.quantity) && (
                            <p className="mt-1 text-xs text-rose-600">Required</p>
                          )}
                        </div>
                        <div>
                          ...
                          <div className="w-full px-2 py-1.5 border rounded-md text-sm border-slate-300 bg-slate-100 text-slate-700 flex items-center">
                            <span>{item.estimated_unit_cost ? `₹${Number(item.estimated_unit_cost).toFixed(2)}` : '₹0.00'}</span>
                          </div>
                        </div>
                        <div>
