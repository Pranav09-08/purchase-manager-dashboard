...
function PurchaseManagerDashboard(props) {
  ...
  const handleSubmitEnquiry = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || error.message || 'Failed to save enquiry');
    }
  };
  ...
  const handleRequestNegotiation = async (quotationId) => {
    const notes = prompt('Enter negotiation notes:');
    if (!notes) return;
    ...
    try {
      ...
      alert('Quotation marked for negotiation successfully');
    } catch (error) {
      alert(error.response?.data?.error || error.message || 'Failed to negotiate quotation');
    }
  };
  ...
  const handleGoToLois = async (quotationOrCounter) => {
    ...
    const counterId = quotationOrCounter.counter_id || quotationOrCounter.counterId;
    
    try {
      // If it's a quotation, fetch its details to pre-fill LOI form
      if (quotationId) {
        const token = idToken || await getIdToken(true);
        const quotationData = await getQuotation(token, quotationId);
        
        // Pre-fill LOI form with quotation details
        setLoiFormData(prev => ({
          ...prev,
          quotationId: quotationId || '',
          counterQuotationId: counterId || '',
          totalAmount: quotationData.quotation?.total_amount || quotationOrCounter.total_amount || '',
          expectedDeliveryDate: quotationData.quotation?.expected_delivery_date || quotationOrCounter.expected_delivery_date || '',
          advancePaymentPercent: quotationData.quotation?.advance_payment_percent || quotationOrCounter.advance_payment_percent || '',
        }));
      } else {
        setLoiFormData(prev => ({
          ...prev,
  ...
  const handleClearFocus = () => {
    // Clear any focus state if needed
  };
  ...
  const handleSelectQuotation = (quotation) => {
    setLoiFormData(prev => ({
      ...prev,
      quotationId: quotation.quotation_id || quotation.quotationId || '',
      counterQuotationId: '',
    }));
  };
  ...
  const handleSelectCounter = (counter) => {
    setLoiFormData(prev => ({
      ...prev,
      counterQuotationId: counter.counter_id || counter.counterId || '',
      quotationId: counter.quotation_id || counter.quotationId || '',
    }));
  };
  ...
  const handleSubmitLoi = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        advancePaymentPercent: loiFormData.advancePaymentPercent,
  ...
  const handleEditLoi = (loi) => {
    ...
    setLoiFormData({
      ...
      advancePaymentPercent: loi.advance_payment_percent || loi.advancePaymentPercent || '',
  ...
  const handleGoToPayments = (invoice) => {
  ...
  const handleCreatePayment = async () => {
    try {
      ...
      const payload = {
        order_id: paymentFormData.orderId,
  ...
  // Generate component lookups
  const componentLookup = productComponents.reduce((acc, comp) => {
    const key = comp.component_code || comp.componentCode || comp.id;
    acc[key] = comp;
    return acc;
  }, {});
